...
using PAK_DR1.Stream.Emulator.Pak;

namespace PAK_DR1.Stream.Emulator
{
    internal class PakEmulator : IEmulator
    {
        ...
        private void DumpFile(string filepath, MultiStream stream)
        {
            ...
            _log.Info($"[{nameof(PakEmulator)}] Dumping {filepath}");
            ...
            fileStream.Close();
            _log.Info($"[{nameof(PakEmulator)}] Written To {filePath}");
        }
    }
}
